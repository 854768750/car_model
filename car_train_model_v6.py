...
f = open(project_dir+"/state_command_data_8.txt", "r")
next(f)
result = []
for line in f:
...
input_data = np.concatenate((theta_t,command_data[:,:].reshape([-1,2])),axis=1)
...
if train==1:
    ...
    for i in range(1,iterations): 
        ...
        if loss_data<lowest_loss:
            ...
            saver.save(sess, project_dir+'/ckpt_v6/model.ckpt', global_step=global_step)
        if i % (1000) == 0: 
    ...
    sess = tf.InteractiveSession()
    model_file=tf.train.latest_checkpoint(project_dir+'/ckpt_v6')
...
for i in range(0,predict_size,1):
    ...
    current_input = np.concatenate((theta_t[start+i,:].reshape([-1,1]), command_data[start+i,:].reshape([-1,2])),axis=1)
